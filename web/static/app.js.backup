class PGWManager {
  constructor() {
    ...
    try {
      const sp = JSON.parse(localStorage.getItem('pgw_sort_p_ui')||'{}');
      if (sp && sp.k) { this.pSort = sp.k; this.pAsc = !!sp.a; }
      const sm = JSON.parse(localStorage.getItem('pgw_sort_m_ui')||'{}');
    ...
    if (thead) {
      ...
      thead.querySelectorAll('th.sortable').forEach((th)=>{
        th.style.cursor='pointer'; th.onclick=()=>{
          ...
          localStorage.setItem('pgw_sort_p_ui', JSON.stringify({k:this.pSort,a:this.pAsc}));
          this.renderProxies();
        };
      });
    }
    ...
    if (thead) {
      ...
      thead.querySelectorAll('th.sortable').forEach((th)=>{
        th.style.cursor='pointer'; th.onclick=()=>{
          ...
          localStorage.setItem('pgw_sort_m_ui', JSON.stringify({k:this.mSort,a:this.mAsc}));
          this.renderMappings();
        };
      });
    }
