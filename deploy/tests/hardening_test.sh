...
assert_not_contains "${ROOT}/deploy/install-pgw.sh" '"${backup_dir}/metadata.json"'
assert_contains "${ROOT}/deploy/install-pgw.sh" '"${metadata}" == "${snapshot_restore_stage}/metadata.json"'
assert_contains "${ROOT}/deploy/install-pgw.sh" 'atomic_restore_path absent /dev/null "${target}" "${restore_stage}/metadata.json" "${path}"'
...
assert_contains "${ROOT}/deploy/tests/installer_transaction_test.sh" 'systemd-sysctl absent/present fixture fidelity'
assert_contains "${ROOT}/deploy/tests/lifecycle_fake.sh" '[[ ! -L "${config}" ]] || return 2'
assert_contains "${ROOT}/deploy/tests/lifecycle_fake.sh" '[[ -f "${config}" ]] || return 2'
assert_contains "${ROOT}/deploy/tests/lifecycle_fake.sh" '"${config_links}" == 1'
assert_contains "${ROOT}/deploy/tests/lifecycle_fake.sh" 'config_value="$(/usr/bin/awk'
assert_contains "${ROOT}/deploy/tests/installer_transaction_test.sh" 'PGW sysctl last active assignment did not win'
assert_contains "${ROOT}/deploy/tests/installer_transaction_test.sh" 'assert_fake_sysctl_rejected "${fixture}" "${command}" hardlink'
